...
def main():
    ...
    text = "Maximum length sequence code PSR evaluation of degree " + str(deg)
    figs_mseq.update_layout(title_text = text, showlegend=False)
    #figs_mseq.show()
    figs_mseq.write_image("img/mseq_eva_psr.pdf", scale=1, width= 2.5 * DPIEXPORT, height= 1 * DPIEXPORT)

    figs_mseq = make_subplots(rows=1, cols=2, subplot_titles=('ACR per polynomial [oct]', 'autocorrelation of max ACR'))
    ...
    text = "Maximum length sequence code ACR evaluation of degree " + str(deg)
    figs_mseq.update_layout(title_text = text, showlegend=False)
    #figs_mseq.show()
    ...
    text = "Gold code PSR evaluation of degree " + str(deg)
    figs_gold.update_layout(title_text = text, showlegend=False)
    #figs_gold.show()
    figs_gold.write_image("img/gold_eva_acr.pdf", scale=1, width= 2.5 * DPIEXPORT, height= 1 * DPIEXPORT)
    figs_gold = make_subplots(rows=1, cols=2, subplot_titles=('ACR per polynomial [oct]', 'autocorrelation of max ACR'))
    ...
    text = "Gold code ACR evaluation of degree " + str(deg)
    figs_gold.update_layout(title_text = text, showlegend=False)
    #figs_gold.show()
    figs_mseq.write_image("img/gold_eva_psr.pdf", scale=1, width= 2.5 * DPIEXPORT, height= 1 * DPIEXPORT)
